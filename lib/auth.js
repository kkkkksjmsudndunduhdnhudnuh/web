...
// Token: <base64url username>.<password version>.<expiry>.<signature>
function createToken(username, version) {
  const payload = `${Buffer.from(username).toString('base64url')}.${version}.${Date.now() + MAX_AGE_MS}`;
  return `${payload}.${sign(payload)}`;
}

// Returns { username, version } for a valid, unexpired token, otherwise null.
function verifyToken(token) {
  ...
  const [user, version, expiry] = payload.split('.');
  if (!user || !(Date.now() < Number(expiry))) return null;
  return { username: Buffer.from(user, 'base64url').toString(), version: Number(version) };
}

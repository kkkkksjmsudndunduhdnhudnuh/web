...
// Faellt bei Redis-Fehlern auf den Speicher zurueck, damit ein Datenbankproblem
// nicht Login und Terminanfragen blockiert.
async function hit(key, limit, windowSeconds) {
  if (redis) {
    try {
      return await hitRedis(key, limit, windowSeconds);
    } catch (err) {
      console.error('Rate-Limit: Redis nicht erreichbar, nutze Speicher-Fallback:', err.message);
    }
  }
